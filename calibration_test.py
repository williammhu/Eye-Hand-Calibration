"""
Planar hand-eye calibration using a USB camera + ArUco markers.

This script replaces the previous depth-based Orbbec pipeline with a simpler
2D homography approach suitable for Raspberry Pi + USB webcam + Freenove robot
arm.  The workflow:

1) Attach an ArUco marker to the robot gripper.
2) Provide a list of robot table coordinates to sample (in millimetres).
3) For each point the script moves the robot, detects the marker in the image,
   and records the pixel/robot correspondence.
4) OpenCV computes a homography H (image pixel -> robot XY).  H and its inverse
   are saved to ``save_parms/homography.npy`` / ``homography_inv.npy``.
5) In "follow" mode the script detects the marker in real time and drives the
   robot to the corresponding XY on the table with a fixed Z height.

Usage examples
--------------
Calibrate with default points (dry run, no hardware commands):
    python calibration_test.py --mode calibrate --dry-run

Follow the marker after calibration (assumes homography files exist):
    python calibration_test.py --mode follow --z-height 70
"""
...
# Default ArUco dictionary and marker size (in meters, used only for axis drawing)
...
class PlaneCalibrator:
    ...
    def detect_marker(self, frame: np.ndarray) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        # Draw an axis for visual feedback (camera matrix is approximated for display only)
        ...
        # Compute the first marker center
        corner = corners[0][0]
...
def default_calibration_points(z_height: float) -> List[Tuple[float, float, float]]:
    """Square grid of points for homography estimation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Planar hand-eye calibration with USB camera and ArUco marker")
    ...
    parser.add_argument("--host", type=str, default="127.0.0.1", help="Freenove server host")
    parser.add_argument("--port", type=int, default=20001, help="Freenove server port")
    parser.add_argument("--dry-run", action="store_true", help="Print robot commands instead of sending them")
    parser.add_argument("--speed", type=int, default=50, help="Robot move speed for follow mode")
...
def main() -> None:
    ...
    with FreenoveArmClient(host=args.host, port=args.port, dry_run=args.dry_run) as arm:
        if args.mode == "calibrate":
            robot_points = default_calibration_points(args.z_height)
            img_pts, rob_pts = calibrator.collect_correspondences(robot_points, arm)
